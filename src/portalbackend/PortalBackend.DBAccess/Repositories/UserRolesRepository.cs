...
public class UserRolesRepository : IUserRolesRepository
{
    ...
    public IAsyncEnumerable<UserRoleModificationData> GetAssignedAndMatchingOfferRoles(Guid companyUserId, IEnumerable<string> userRoles, Guid offerId) =>
        _dbContext.UserRoles
            .AsNoTracking()
            .Where(role =>
    ...
    public async IAsyncEnumerable<UserRoleData> GetOwnCompanyPortalUserRoleDataUntrackedAsync(IDictionary<string, IEnumerable<string>> clientRoles, string iamUserId)
    {
        foreach (var clientRole in clientRoles)
        {
            await foreach (var userRoleData in _dbContext.UserRoles
                .AsNoTracking()
                .Where(userRole => userRole.Offer!.AppInstances.Any(ai => ai.IamClient!.ClientClientId == clientRole.Key) &&
                    clientRole.Value.Contains(userRole.UserRoleText) &&
                    userRole.UserRoleCollections.Any(collection => collection.CompanyRole.Companies.Any(company => company.CompanyUsers.Any(companyUser => companyUser.IamUser!.UserEntityId == iamUserId))))
                .Select(userRole => new {
                    Id = userRole.Id,
                    Text = userRole.UserRoleText
                })
