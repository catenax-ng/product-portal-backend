...
public class UserProvisioningService : IUserProvisioningService
{
    ...
    public async IAsyncEnumerable<UserRoleData> GetRoleDatas(IDictionary<string,IEnumerable<string>> clientRoles)
    {
        ...
        {
            ValidateRoleData(roleDataGrouping, clientRoles);
    ...
    public async IAsyncEnumerable<UserRoleData> GetOwnCompanyPortalRoleDatas(IDictionary<string,IEnumerable<string>> clientRoles, string iamUserId)
    {
        await foreach (var roleDataGrouping in _portalRepositories.GetInstance<IUserRolesRepository>()
            .GetOwnCompanyPortalUserRoleDataUntrackedAsync(clientRoles, iamUserId)
            .PreSortedGroupBy(d => d.ClientClientId))
        {
            ValidateRoleData(roleDataGrouping, clientRoles);
            foreach (var data in roleDataGrouping)
    ...
    private static void ValidateRoleData(IGrouping<string,UserRoleData> roleData, IDictionary<string,IEnumerable<string>> clientRoles)
    {
        var invalid = clientRoles[roleData.Key]
            .Except(roleData.Select(r => r.UserRoleText));

        if (invalid.Any())
        {
            throw new ControllerArgumentException($"invalid roles: {String.Join(", ",invalid.Select(role => $"clientId: {roleData.Key}, role: {role}"))}");
        }
    }
}
