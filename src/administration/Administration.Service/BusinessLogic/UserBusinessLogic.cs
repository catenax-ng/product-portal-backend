...
public class UserBusinessLogic : IUserBusinessLogic
{
    ...
    public async IAsyncEnumerable<Guid> DeleteOwnCompanyUsersAsync(IEnumerable<Guid> companyUserIds, string iamUserId)
    {
        ...
        await foreach (var accountData in _portalRepositories.GetInstance<IUserRepository>().GetCompanyUserAccountDataUntrackedAsync(companyUserIds, adminUserId, _settings.CompanyUserStatusIds).ConfigureAwait(false))
        {
            var success = false;
            try
            {
