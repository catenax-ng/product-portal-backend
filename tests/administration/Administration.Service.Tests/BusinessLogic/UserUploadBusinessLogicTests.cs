...
public class UserUploadBusinessLogicTests
{
    ...
    private void SetupFakes(IEnumerable<string> lines)
    {
        ...
        A.CallTo(() => _userProvisioningService.GetOwnCompanyPortalRoleDatas(A<IDictionary<string,IEnumerable<string>>>._, A<string>._))
            .ReturnsLazily((IDictionary<string,IEnumerable<string>> clientRoles, string _) =>
                clientRoles.SelectMany(r => r.Value.Select(role => _fixture.Build<UserRoleData>().With(x => x.ClientClientId, r.Key).With(x => x.UserRoleText, role).Create())).ToAsyncEnumerable());
