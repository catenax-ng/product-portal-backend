...
public class UserBusinessLogicTests
{
    ...
    private void SetupFakesForUserDeletion()
    {
        ...
        A.CallTo(() => _userRepository.GetCompanyUserAccountDataUntrackedAsync(A<IEnumerable<Guid>>._,A<Guid>._,A<IEnumerable<CompanyUserStatusId>>._))
            .Returns(Enumerable.Empty<CompanyUserAccountData>().ToAsyncEnumerable());
        A.CallTo(() => _userRepository.GetCompanyUserAccountDataUntrackedAsync(A<IEnumerable<Guid>>._,A<Guid>.That.IsEqualTo(_companyUserId),A<IEnumerable<CompanyUserStatusId>>._))
