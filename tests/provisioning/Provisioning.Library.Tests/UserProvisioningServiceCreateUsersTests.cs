...
public class UserProvisioningServiceCreateUsersTests
{
    ...
    public async void TestGetOwnCompanyPortalRoleDatasSuccess()
    {
        ...
        var result = await sut.GetOwnCompanyPortalRoleDatas(clientRoles,iamUserId).ToListAsync().ConfigureAwait(false);
    ...
    public async void TestGetOwnCompanyPortalRoleDatasThrows()
    {
        ...
        A.CallTo(() => _userRolesRepository.GetOwnCompanyPortalUserRoleDataUntrackedAsync(A<IDictionary<string, IEnumerable<string>>>._, A<string>._))
            .ReturnsLazily((IDictionary<string,IEnumerable<string>> clientRoles, string _) =>
                clientRoles.SelectMany(r => r.Value.Take(r.Value.Count()-1).Select(role => _fixture.Build<UserRoleData>()
                    .With(x => x.ClientClientId, r.Key)
                    .With(x => x.UserRoleText, role).Create())).ToAsyncEnumerable());
        ...
        async Task Act() => await sut.GetOwnCompanyPortalRoleDatas(clientRoles,iamUserId).ToListAsync().ConfigureAwait(false);
